...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	noServerCert := hs.suite.flags&suiteNoCerts != 0
	...
	if ok {
		...
		err = keyAgreement.processServerKeyExchange(c.config, hs.hello, hs.serverHello, peerCertificate(c), skx)
		if err != nil {
			c.sendAlert(alertUnexpectedMessage)
			return err
		}
	...
	preMasterSecret, ckx, err := keyAgreement.generateClientKeyExchange(c.config, hs.hello, peerCertificate(c))
	if err != nil {
		c.sendAlert(alertInternalError)
		return err
	}
...
func peerCertificate(c *Conn) *x509.Certificate {
